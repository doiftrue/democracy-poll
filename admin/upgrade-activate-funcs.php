...
function democracy_activate() {
	...
	if( is_admin() && version_compare( PHP_VERSION, '5.3', '<' ) ){
		deactivate_plugins( plugin_basename( DEMOC_MAIN_FILE ) );
	...
	if(
		is_multisite()
		&&
		count( $sites = ( function_exists( 'get_sites' ) ? get_sites() : wp_get_sites() ) ) > 0
	){
...
function _democracy_activate() {
	...
	// переопределим названия таблиц для мультисайта
	dem_set_dbtables();
...
function dem_last_version_up() {
	...
	// 4.5.6
	if( ! in_array( 'expire', $fields_log, true ) ){
